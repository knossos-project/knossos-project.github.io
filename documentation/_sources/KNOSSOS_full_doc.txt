...
:term:`Branch points` help you to relocate important nodes in your skeleton.
...
Add a Branch Point
^^^^^^^^^^^^^^^^^^
* You can make a node a branch point by simply pressing :kbd:`B` on your keyboard while the node is activated. The color of the node turns to blue.
* Another possibility is to use :kbd:`Ctrl-(RB)` on a new position. A node will be added which is simultaneously marked as a branch point. This node does not automatically become active (in comparison to normal node placing).

Jump to a Branch Point
^^^^^^^^^^^^^^^^^^^^^^

* Press :kbd:`J` on your keyboard to jump to the last made branch point. The flagging as a branch point is automatically removed. If you want to keep it, press :kbd:`B` again on your keyboard.
* Another possibility is to click on :guilabel:`Pop and (J)ump` in the *Annotation window.*
...
:kbd:`Ctrl-Q` Close KNOSSOS (equal to :kbd:`Alt-F4`)
============= ===========================
...
:kbd:`Z/Shift+Z` Move to next/previous tree (by tree ID)
:kbd:`X/Shift+X` Move to next/previous node (by node ID)
================ =======================================
...
:kbd:`Shift+LMB` on node          Mark clicked node as active node
:kbd:`Ctrl+LMB` on node	          Select node for an operation
:kbd:`Ctrl+LMB` drag	          Pull selection square to select multiple nodes
:kbd:`Alt+LMB` on node            Add/delete segment between clicked node and
                                  active node 
...
Shortcut                           Action
================================== =============================================
:kbd:`←/→`                         Jump [Jump Frames] steps left/right in
                                   active viewport (Jump Frames can be defined
                                   at “Preferences” – “Dataset Navigation”)
:kbd:`Shift-←/→`                   Jump 10 steps left/right in active viewport
:kbd:`↑/↓`                         Jump [Jump Frames] steps up/down in active 
                                   viewport
:kbd:`Shift-↑/↓`                   Jump 10 steps up/down in active viewport
:kbd:`F/D`                         Jump [Jump Frames] steps forward/backward in 
                                   active viewport (Jump Frames can be defined 
                                   at “Preferences” – “Dataset Navigation”)

                                   (equal to mouse wheel forward/backward)

:kbd:`Shift-F/D`                   Jump 10 steps forward/backward in active
                                   viewport
:kbd:`I/O` in orthogonal viewports Zoom in and out of orthogonal viewports
                                   (equal to Ctrl + mouse scroll)
Ctrl+Alt + Left Mouse drag         drag viewport (restore original position by 
                                   clicking on “Reset VP Positions” in the 
                                   toolbar)
...
* Use the arrow keys on your keyboard to move into the corresponding direction (depending on which slice viewport your mouse cursor is currently positioned).
	...
	* Using the arrow keys while holding down :kbd:`Shift` will move 10 planes.
...
* Using D and F on your keyboard will also move the dataset perpendicularly an amount of [Jump Frames] planes, [Jump Frames] can be defined under “Preferences” –> “Navigation Settings”.
	...
	* Using :kbd:`F/D` while holding down :kbd:`Shift` will perpendicularly move 10 planes.

* Placing a node using your right mouse button will recenter the view on the node. (Adding one using :kbd:`Ctrl` + right mouse button won’t do so).
* You can jump to the currently active node using :kbd:`S` on your keyboard.
...
* You can jump to a node previously marked as a branch point using :kbd:`J` on your keyboard or using your Annotation window.
* You can search for nodes with comments using :kbd:`N` and :kbd:`P` on your keyboard or your *Annotation window*.
...
You can connect two nodes with each other by activating one of the two nodes and clicking on the other one with the middle mouse button while simultaneously holding down the :kbd:`Ctrl` key on your keyboard.
...
You can delete a node by selecting it (:kbd:`Ctrl`+left click) and pressing :kbd:`Del` on your keyboard.
...
* Sometimes you want to perform actions on multiple nodes, e.g. delete many nodes at once. You do this by selecting nodes in the viewport: :kbd:`Ctrl`\ +left-click on a node selects it. :kbd:`Ctrl`\ +left mouse drag in the viewport pulls a selection square that selects all nodes within.
* :kbd:`Ctrl`\ +left-click a selected node to unselect it, or press :kbd:`Esc:kbd:` to undo the entire node selection.
* Press :kbd:`Del` to delete all selected nodes.
...
Under :guilabel:`Windows` :guilabel:`Annotation Window` you can open an important, additional window with many features.
